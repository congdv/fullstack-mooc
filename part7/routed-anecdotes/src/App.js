    ...
    <div>
      <Router>
        <div>
          ...
          <Route exact path="/anecdotes/:id" render={({match}) => <Anecdote anecdote={anecdoteById(anecdotes, match.params.id)}/>}/>
...
const Anecdote = ({anecdote}) => {
  ...
  return (
    <div>
      ...
      <p>has {anecdote.votes} {anecdote.votes > 1? "votes": "vote"}</p>
      <p>for more infor see <a href="{anecdote.info}">{anecdote.info}</a></p>
    </div>
  )
}
const anecdoteById = (anecdotes, id) => anecdotes.find(anecdote => anecdote.id == Number(id))
